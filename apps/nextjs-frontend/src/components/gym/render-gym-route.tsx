  ...
  return (
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <SectionCard title="Low stock alerts" description="Cac san pham can uu tien restock trong ngay.">
          <DataTable
            headers={['Product', 'Stock', 'Threshold', 'Status']}
            rows={sortProductsByStock(snapshot.dashboard.lowStockProducts).map((product) => [
              product.name,
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <SectionCard title="Maintenance alerts" description="Thiet bi can bao tri hoac thay the som.">
          <DataTable
            headers={['Equipment', 'Condition', 'Next maintenance', 'Action']}
            rows={sortEquipmentByMaintenance(snapshot.dashboard.maintenanceAlerts).map((equipmentAsset) => [
              equipmentAsset.name,
              <Badge key={equipmentAsset.id} tone={getStatusTone(equipmentAsset.condition)}>
                {humanizeStatus(equipmentAsset.condition)}
              </Badge>,
              formatDate(equipmentAsset.nextMaintenanceAt),
              <ActionLink key={`${equipmentAsset.id}-action`} href={`/equipment/${equipmentAsset.id}`}>
                Open asset
              </ActionLink>,
            ])}
          />
        ...
        <SectionCard title="Top selling products" description="San pham dang tao doanh so cao nhat trong ky hien tai.">
          <DataTable
    ...
    <>
      ...
      <StatsGrid
        items={[
          ...
          {
            label: 'Overtime',
            value: formatHours(snapshot.ptOverview.reduce((total, item) => total + item.overtimeHours, 0)),
            note: 'Tong gio tang ca duoc lay tu attendance logs.',
          },
          {
            label: 'Estimated payroll',
            value: formatCurrency(snapshot.ptOverview.reduce((total, item) => total + item.estimatedPayroll, 0)),
            note: 'Tong net pay ky gan nhat cua tat ca PT.',
          },
        ]}
      />

      <SectionCard title="PT roster" description="Mo tung profile de xem contract, attendance va payroll chi tiet.">
        <DataTable
          headers={['PT', 'Specialties', 'Active members', 'Shift credits', 'Overtime', 'Net pay', 'Detail']}
          rows={snapshot.ptOverview.map((item) => [
            ...
            <div key={`${item.pt.id}-links`} className="flex gap-2">
              ...
              <ActionLink href={`/pts/${item.pt.id}/contracts`}>Contract</ActionLink>
            </div>,
          ])}
        />
      </SectionCard>
...
function buildPtDetailPage(ptId: string): JSX.Element {
  ...
  const assignedMembers = snapshot.dataset.memberPtAssignments
    ...
    .map((assignment) => snapshot.dataset.members.find((member) => member.id === assignment.memberId))
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<ActionLink href={`/pts/${ptId}/contracts`}>View contract</ActionLink>}
      />
        ...
        <SectionCard title="Assigned members">
          <DataTable
            ...
            rows={assignedMembers.map((member) => [
              ...
              <ActionLink key={`${member.id}-detail`} href={`/members/${member.id}`}>
                Open member
              </ActionLink>,
            ])}
          />
      ...
      <SectionCard title="Attendance timeline">
        <DataTable
          headers={['Date', 'Check in', 'Check out', 'Worked', 'Overtime', 'Status']}
          rows={attendanceLogs.map((attendanceLog) => [
            attendanceLog.attendanceDate,
            formatDateTime(attendanceLog.checkInAt),
            attendanceLog.checkOutAt ? formatDateTime(attendanceLog.checkOutAt) : 'Open',
            formatHours(attendanceLog.workedHours),
            formatHours(attendanceLog.overtimeHours),
            <Badge key={attendanceLog.id} tone={getStatusTone(attendanceLog.status)}>
      ...
      <SectionCard title="Payroll history">
        <DataTable
          headers={['Period', 'Shift credits', 'Overtime', 'Package commission', 'Net pay', 'Status']}
  ...
  return (
    <>
      ...
      <StatsGrid
        items={[
          ...
          },
          {
            label: 'Overtime',
            value: formatHours(
              snapshot.dataset.attendanceLogs.reduce((total, attendanceLog) => total + attendanceLog.overtimeHours, 0),
            ),
      ...
      <SectionCard title="Attendance table">
        <DataTable
          headers={['PT', 'Date', 'Check in', 'Check out', 'Worked', 'Overtime', 'Status']}
        ...
        <SectionCard title="Payroll periods">
          <DataTable
            ...
            rows={snapshot.dataset.payrollPeriods.map((period) => [
              ...
              <ActionLink key={`${period.id}-detail`} href={`/payroll/${period.id}`}>
                Open period
              </ActionLink>,
            ])}
          />
        ...
        <SectionCard title="Payroll by trainer">
          <DataTable
            ...
            rows={snapshot.payrollReport.byTrainer.map((item) => [
              ...
              <Badge key={`${item.ptId}-${item.payrollPeriodId}`} tone={getStatusTone(item.status)}>
  ...
  return (
    <>
      ...
      <SectionCard title="Entries in period">
        <DataTable
          headers={['PT', 'Shift credits', 'Overtime', 'Package commission', 'Sales commission', 'Net pay', 'Status']}
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<ActionLink href="/members/memberships">Open sold memberships</ActionLink>}
      />
      ...
      <SectionCard title="Member roster">
        <DataTable
          headers={['Member', 'Current plan', 'Assigned PT', 'Membership spend', 'Service spend', 'Detail']}
    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <SectionCard title="Membership history">
          <DataTable
            headers={['Plan', 'Start', 'End', 'Remaining PT sessions', 'Status']}
            rows={memberships.map((membership) => [
              ...
              <Badge key={membership.id} tone={getStatusTone(membership.status)}>
      ...
      <SectionCard title="PT assignments">
        <DataTable
          ...
          rows={ptAssignments.map((assignment) => [
            ...
            assignment.assignedTo ? formatDate(assignment.assignedTo) : 'Active now',
...
function buildMembershipPlansPage(): JSX.Element {
  ...
  return (
    <>
      <PageHeader eyebrow="Catalog" title="Membership plans" description="Danh muc plan DAY / MONTH / YEAR va perks di kem cho tung goi." />
...
function buildMembershipInvoicesPage(): JSX.Element {
  ...
  return (
    <>
      <PageHeader eyebrow="Membership billing" title="Membership invoices" description="Danh sach hoa don membership da tao khi member mua hoac gia han goi tap." />
  ...
  return (
    <>
      ...
      <SectionCard title="Product list">
        <DataTable
          headers={['Product', 'Category', 'Unit cost', 'Sale price', 'Stock', 'Threshold']}
          rows={sortProductsByStock(snapshot.dataset.products).map((product) => [
            product.name,
            product.category,
...
function buildInventoryImportPage(): JSX.Element {
  ...
  return (
    <>
      <PageHeader eyebrow="Restocking" title="Import tracker" description="Tap trung vao cac giao dich nhap kho va danh sach san pham can mua them." />
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <SectionCard title="Suggested restock queue">
          <DataTable
            headers={['Product', 'Current stock', 'Threshold', 'Suggested action']}
            rows={sortProductsByStock(snapshot.dashboard.lowStockProducts).map((product) => [
              product.name,
              `${product.stockOnHand}`,
              `${product.minimumStockLevel}`,
...
function buildInvoicesPage(): JSX.Element {
  ...
  return (
    <>
      <PageHeader eyebrow="Sales" title="Service invoices" description="Hoa don ban san pham dich vu cho member hoac khach le." />
      <SectionCard title="Sales invoices">
        <DataTable
          headers={['Code', 'Customer', 'Date', 'Total', 'Payment', 'Status', 'Detail']}
          rows={snapshot.dataset.salesInvoices.map((invoice) => [
            ...
            <ActionLink key={`${invoice.id}-detail`} href={`/invoices/${invoice.id}`}>
              Open invoice
            </ActionLink>,
          ])}
        />
...
function buildInvoiceDetailPage(invoiceId: string): JSX.Element {
  ...
  return (
    <>
      <PageHeader eyebrow="Sales invoice detail" title={invoice.code} description={`${invoice.customerName} | ${formatDateTime(invoice.invoiceDate)}`} actions={<ActionLink href="/invoices">Back to invoices</ActionLink>} />
      ...
      <SectionCard title="Invoice items">
        <DataTable
          ...
          rows={invoice.items.map((item, index) => [
            ...
            <span key={`line-${index}`} className="font-semibold text-slate-900">
  ...
  return (
    <>
      ...
      <SectionCard title="Expense slips">
        <DataTable
          headers={['Code', 'Date', 'Category', 'Equipment', 'Amount', 'Status', 'Detail']}
          rows={snapshot.dataset.operatingExpenses.map((expense) => [
            expense.code,
            formatDate(expense.expenseDate),
            expense.category,
            getEquipmentName(snapshot, expense.equipmentAssetId),
            formatCurrency(expense.amount),
            <Badge key={expense.id} tone={getStatusTone(expense.status)}>
              {humanizeStatus(expense.status)}
            </Badge>,
            <ActionLink key={`${expense.id}-detail`} href={`/expenses/${expense.id}`}>
              Open slip
            </ActionLink>,
          ])}
        />
...
function buildExpenseDetailPage(expenseId: string): JSX.Element {
  ...
  return (
    <>
      <PageHeader eyebrow="Expense detail" title={expense.code} description={`${expense.category} | ${expense.vendorName}`} actions={<ActionLink href="/expenses">Back to expenses</ActionLink>} />
  ...
  return (
    <>
      ...
      <SectionCard title="Equipment list">
        <DataTable
          headers={['Equipment', 'Purchased', 'Value', 'Condition', 'Next maintenance', 'Detail']}
          rows={sortEquipmentByMaintenance(snapshot.dataset.equipmentAssets).map((equipmentAsset) => [
            equipmentAsset.name,
            ...
            formatDate(equipmentAsset.nextMaintenanceAt),
            <ActionLink key={`${equipmentAsset.id}-detail`} href={`/equipment/${equipmentAsset.id}`}>
              Open asset
            </ActionLink>,
          ])}
        />
...
function buildEquipmentDetailPage(equipmentId: string): JSX.Element {
  ...
  return (
    <>
      <PageHeader eyebrow="Equipment detail" title={equipmentAsset.name} description={equipmentAsset.note} actions={<ActionLink href="/equipment">Back to equipment</ActionLink>} />
...
function buildMaintenancePage(): JSX.Element {
  ...
  return (
    <>
      <PageHeader eyebrow="Maintenance log" title="Maintenance history" description="Tat ca event bao tri, repair va recommendation replacement cho thiet bi." />
      <SectionCard title="Maintenance records">
        <DataTable
          ...
          rows={snapshot.dataset.maintenanceRecords.map((record) => [
            formatDate(record.maintenanceDate),
            getEquipmentName(snapshot, record.equipmentAssetId),
            record.vendorName,
            record.description,
            formatCurrency(record.amount),
          ])}
        ...
        <SectionCard title="Retail invoices">
          <DataTable
            ...
            rows={snapshot.dataset.salesInvoices
              ...
              .map((invoice) => [invoice.code, invoice.customerName, formatDateTime(invoice.invoiceDate), formatCurrency(invoice.totalAmount)])}
          />
        </SectionCard>
      </div>
    </>
    ...
    <>
      ...
      <SectionCard title="Expense by category">
        <DataTable
          ...
          rows={Object.entries(snapshot.expenseReport.byCategory).map(([category, amount]) => [category, formatCurrency(amount)])}
        />
      </SectionCard>
    </>
  );
...
function buildSettingsPage(): JSX.Element {
  ...
  return (
    <>
      <PageHeader eyebrow="System config" title="Settings" description="Cac tham so policy can Admin co the chinh sua trong he thong." />
    ...
    <>
      ...
      <SectionCard title="Demo accounts">
        <DataTable
          ...
          rows={snapshot.dataset.users.map((user) => [user.role, user.fullName, user.email, user.passwordHint])}
        />
      </SectionCard>
